...
export class MappaUtentiComponent implements OnInit, AfterViewInit {
          ...
          this.authService.fetchUtentiCompleti().subscribe({
            next: (utenti) => {
              this.utenti = utenti.map((u) => {
                const perizieUtente = this.perizieTotali.filter(
                  (p) => p.codiceOperatore?.toString() === u._id
                );
              ...
              this.perizieTotali = tutteLePerizie.filter((p) =>
                utentiId.includes(p.codiceOperatore?.toString())
              );
  ...
  ngAfterViewInit(): void {
    const defaultCenter: [number, number] = [
      7.7361193837862015, 44.55599213612909,
    ]; // Sede
    ...
    new maplibregl.Marker({ element: sedeEl })
      ...
      .setPopup(
        new maplibregl.Popup({ offset: 25 }).setHTML(
          '<div class="text-white">🏢 Sede Ufficio<br>Via San Michele, 68</div>'
        )
      )
      .addTo(this.map);
  }
  ...
  renderMarkers(): void {
    ...
    this.perizieFiltrate.forEach((perizia) => {
      ...
      const user = this.utenti.find(
        (u) => u._id === codiceOperatore.toString()
      );
      ...
      const distanzaKm = this.calcolaDistanza(
        44.55599213612909,
        7.7361193837862015,
        lat,
        lon
